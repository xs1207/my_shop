<?php
    ...
    function sendEmail($address,$code){
        ...
        $mail->Username ='18237243730@163.com';

        //smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
        $mail->Password = 'dkl0614';//163邮箱也有授权码 进入163邮箱帐号获取

        //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
        $mail->From = '18237243730@163.com';
    ...
    function sendSms($tel,$code) {
        ...
        $accessKeyId = "LTAImxwvAe0KW5hA";
        $accessKeySecret = "ZclrH5UkuEcN4N2nljyFAal984uiLi";
        ...
        $params["TemplateCode"] = "SMS_151690598";
    ...
    /** 递归处理 */
    function getNodeCateInfo($nodeInfo,$pid=0){
        $info=[];
        foreach($nodeInfo as $k=>$v){
            if($v['pid']==$pid){
                $son=getNodeCateInfo($nodeInfo,$v['node_id']);
                $v['son']=$son;
                $info[]=$v;
            }
        }
    ...
    /**递归处理节点信息 */
    function getRecursionNodeInfo($nodeInfo,$pid=0){
        $info=[];
        foreach($nodeInfo as $k=>$v){
            if($v['pid']==$pid&&$v['node_type']==1){
                $son=getRecursionNodeInfo($nodeInfo,$v['node_id']);
                $v['son']=$son;
                $info[]=$v;
            }
        }
